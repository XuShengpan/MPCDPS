...
namespace mpcdps {
    ...
    template<typename T>
    void OBRect<T>::getVertices(Point2<T> vertices[4]) const
    {
        mpcdps::Point2<T> pt = _center - _extents[0] * _axises[0] - _extents[1] * _axises[1];
		vertices[0] = pt;

        pt = _center - _extents[0] * _axises[0] + _extents[1] * _axises[1];
        vertices[1] = pt;
